...
async def upload_parts(urls: list[str], data: bytes, upload_id: str, max_retries: int = 3) -> list[dict]:
    ...
    async with aiohttp.ClientSession() as session:
        parts = []
        for i, (part_number, url) in enumerate(zip(range(1, len(urls) + 1), urls)):
        ...
        # TODO: notify orchestrator that parts are uploaded
    return parts
...
async def download_metadata(path: str) -> dict[str, Any]:
