...
class MetagraphSyncer:
    ...
    def __init__(self, subtensor: bt.subtensor):
        ...
        self.metagraph_map: Dict[int, MetagraphSyncer._State] = {settings.netuid: MetagraphSyncer._State()}
        self.is_running = False
        self.done_initial_sync = False
        self.lock = threading.RLock()

        bt.logging.info(f"MetagraphSyncer created with netuid: {settings.netuid}")
    ...
    def do_initial_sync(self):
        ...
        for netuid in [settings.netuid]:
            fn = functools.partial(self.subtensor.metagraph, netuid)
            metagraph = bt_utils.run_in_thread(fn, ttl=120, name=f"InitalSync-{netuid}")
            with self.lock:
    ...
    async def _sync_metagraph_loop(self, netuid: int, cadence: int):
        while self.is_running:
            ...
            try:
                ...
                metagraph = bt_utils.run_in_thread(
    ...
    async def _run_async(self):
        ...
        await asyncio.wait(
            [
                ...
                for netuid, cadence in [[settings.netuid, 100]]
            ],
