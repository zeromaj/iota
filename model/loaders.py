...
from model.llama3.splits import (
    Llama3HeadModel,
    Llama3TailModel,
    Llama3BodyModel,
)

from model.llama3.full import Llama3Model, Llama3ModelNativeBottlenecks

from typing import Union, List


def load_model_split(model_cfg: dict, model_split: Union[list[int], List[List[int]]], device: str, seed: int = 42):
    """
    ...
    # Get the model split from the trainer client
...
def load_dataloader(
    ...
    """
    dataset = load_dataset(dataset_name, split="train", streaming=True, token=settings.HF_TOKEN)
    # shuffle the dataset
...
if __name__ == "__main__":
    import settings
    from model.configs import LLAMA32_CONFIG_1B

    model = load_model_split(
        model_cfg=LLAMA32_CONFIG_1B,
        model_split=settings.MODEL_SPLITS[0],
        device="cuda",
        seed=42,
    )
    print(model)
