...
class ValidatorClient:
    ...
    async def forward_activation(
        self, activation_id: str, direction: Literal["forward", "backward", "initial"]
    ) -> Dict[str, Any]:
        """Perform a forward pass with the gradient validator."""
        params = {"activation_id": activation_id, "direction": direction}
        return await self._make_request("post", f"{self.base_url}/gradient-validator/forward", params=params)

    async def backward_activation(self, activation_id: str) -> Dict[str, Any]:
