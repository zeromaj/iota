...
class StateManager:
    ...
    def add_to_cache(self, activation_uid: str, data: CacheEntry):
        self.cache[activation_uid] = data

    def remove_from_cache(self, activation_uid: str):
        del self.cache[activation_uid]
    ...
    def check_if_timeout(self, timeout: int):
        ...
        if len(self.cache) > 0:
            for activation_uid, activation_data in list(self.cache.items()):
                ...
                if upload_time < time.time() - timeout:
                    ...
                    logger.warning(f"🗑️ Removed activation {activation_uid} from miner cache due to timeout")

        for activation_uid in activations_to_remove:
            self.remove_from_cache(activation_uid)
