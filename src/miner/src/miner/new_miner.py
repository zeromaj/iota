...
from common.models.error_models import LayerStateError, MinerNotRegisteredError
...
from common.utils.partitions import ChunkData, MinerPartition
...
class Miner(BaseNeuron, HealthServerMixin):
        ...
        try:
            while True:
                try:
                    ...
                    if self.state_manager.state == LayerPhase.TRAINING:
                        ...
                        if not self.partitions_submitted:
                            ...
                            if partitions:
                                ...
                                await MinerAPIClient.submit_merged_partitions(
                    ...
                    logger.info(f"🔄 Miner {self.hotkey[:8]} layer state error: {e}")
                    continue
                    ...
                    logger.exception(
                        f"❌ Main loop failed for miner {self.hotkey[:8]} on layer {self.state_manager.layer}: {e}"
                    )
                    await asyncio.sleep(5)
            ...
            if miner_settings.MINER_HEALTH_PORT:
    ...
    async def forward(self, activation: ActivationResponse | None = None):
        ...
        logger.info(
            f"🚀 Starting FORWARD pass for layer {self.state_manager.layer} | Processing activation {activation.activation_uid} | Miner: {self.hotkey[:8]}"
        )
        if self.state_manager.layer == 0:
            ...
            input_activations = download_activation(
        ...
        await MinerAPIClient.submit_activation_request(
        ...
        logger.info(
            f"✅ Successfully completed FORWARD pass for activation {activation.activation_uid} on layer {self.state_manager.layer} | Miner: {self.hotkey[:8]}"
        )
    ...
    async def compute_last_layer_loss(
        ...
        initial_activations_path = await MinerAPIClient.get_targets(
            get_targets_request=GetTargetsRequest(activation_uid=input_activation_response.activation_uid),
            hotkey=self.wallet.hotkey,
        )
        ...
        # TODO: We need to make an extra request to get the targets (initial activation)

        logger.info(
            f"📊 Computed loss {loss:.6f} for activation {input_activation_response.activation_uid} | Layer: {self.state_manager.layer} | Miner: {self.hotkey[:8]}"
        )
        ...
        try:
            response = await MinerAPIClient.report_loss(
                ...
                loss_report=LossReportRequest(
                    activation_uid=input_activation_response.activation_uid, loss=float(loss)
                ),
            )
            response = await self.parse_response(response)
        ...
        self.state_manager.add_to_cache(
            input_activation_response.activation_uid,
            CacheEntry(
                input_activations=input_activations,
    ...
    async def backward(
        ...
        logger.info(
            f"🔄 Starting BACKWARD pass for activation {activation.activation_uid} | Layer: {self.state_manager.layer} | Miner: {self.hotkey[:8]}"
        )

        # Check if activation is in cache
        if activation.activation_uid not in self.state_manager.cache:
        ...
        if self.state_manager.layer != common_settings.N_LAYERS - 1 and common_settings.N_LAYERS > 1:
            ...
            activation_grads: torch.Tensor = download_activation(
                path=activation.presigned_download_url, device=miner_settings.DEVICE
            )
            if not common_settings.MOCK:
        ...
        cached_activations = self.state_manager.cache[activation.activation_uid]
        ...
        # Remove from cache
        self.state_manager.remove_from_cache(activation.activation_uid)
        logger.info(
            f"✅ Successfully completed BACKWARD pass for activation {activation.activation_uid} | Layer: {self.state_manager.layer} | Miner: {self.hotkey[:8]}"
        )
    ...
    async def upload_tensor(
        ...
        data = data.cpu().detach().numpy().tobytes()
    ...
    async def submit_weights(self):
        ...
        try:
            ...
            for name, tensor in {"weights": weights, "optimizer_state": flattened_optimizer_state}.items():
                check_for_nans_and_infs(tensor=tensor, name=f"{name} for miner {self.hotkey[:8]}")
                ...
                tensor_bytes = tensor_cpu.float().numpy().tobytes()
    ...
    async def run_miner(self):
        ...
        if miner_settings.LAUNCH_HEALTH:
            ...
            logger.warning("⚠️ Miner healthcheck API not configured in settings (MINER_HEALTH_PORT missing). Skipping.")
        ...
        for partition in partitions:
            ...
            for packet in weight_path_per_layer:
                ...
                try:
                    ...
                    logger.exception(
                        f"Error downloading chunk {partition.chunk_number} from {weights_path} and {weight_metadata_path}: {e}"
                    )
                    continue
            ...
            partition.weight_metadata_path = weight_metadata_path
            partition.optimizer_state_path = optimizer_state_upload_response.object_path
            partition.optimizer_state_metadata_path = optimizer_state_metadata_path
    ...
    async def parse_response(self, response: dict):
        ...
        if error_name := response["error_name"]:
            if error_name == LayerStateError.__name__:
                logger.error(f"Layer state error: {response['error_dict']}")
    ...
    async def wait_for_state(self, state: LayerPhase):
        while True:
            ...
            if response == state.value:
                ...
                raise LayerStateException(
                    f"Miner {self.hotkey[:8]} is out of sync with the orchestrator, setting state to training"
                )
